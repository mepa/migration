...
$Redshift = ucfirst($redshift);
$folder = $Galaxy.$Redshift;
...
print OFILE "\\multirow\{4\}\{\*\}\{$galaxy $redshift-z $run\}\n";
...
for ($Ms = 1.0e4; $Ms < 1.0e8; $Ms=$Ms*10) 
{
    ...
    open(IFILE, "< /data1/r900-3/mepa/Migration/Data/$run/$folder/$infile");
    @ilines = <IFILE>;
    close(IFILE);
