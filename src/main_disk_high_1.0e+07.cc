...
const double R_e = 1000.0 * pc;
const double n = 1.0;
const double mass = 5.0e+09 * M_sun;

const double r_min = 1.0 * pc;
const double r_max = 50.0 * kpc;

const double mass_halo = 5.0e+10 * M_sun;
const double concentration = 3.5;
const double r_vir = 20.0 * kpc;

const double M_min = 100.0 * M_sun;
const double M_max = 1.0e+07 * M_sun;
...
struct Model
{
  Model(double _R_e, double _n, double _mass) : 
    R_e(_R_e), n(_n), inverse_n(1.0 / n), mass(_mass) 
  {
    double Gamma = gsl_sf_gamma( 2.0 * n );
    normalization = mass / (two_pi * n * sq(R_e) * Gamma );
  }
  ...
  {
    if (n == 1.0) return normalization * exp( - R / R_e);
    else return normalization * exp(- pow(R / R_e, inverse_n));
  }

  double M(double R) const
  {
    if (n == 1.0) return mass * (1.0 - exp(- R / R_e) * (1.0 + R / R_e));
    else 
      {
...
struct Profile 
{
  ...
  vector<double> r, Sigma_baryon, Sigma_total, rho_NFW, Sigma_NFW, mass_baryon, mass_total; //, Omega, dOmega;
  ...
  Profile(const Model & _model, const NFW & _nfw,
    ...
    model(_model), nfw(_nfw), r(c), Sigma_baryon(c), Sigma_total(c), rho_NFW(c), Sigma_NFW(c), mass_baryon(c), mass_total(c), //Omega(c), dOmega(c), 
    mass_nucleus(0.0), evolving_mass_profile(c, 0.0)
  {
	...
	mass_total[i] = mass_baryon[i] + nfw.M(r[i]); //+ mass_nucleus;

	//	Omega[i] = sqrt(G * mass_total[i] / (r[i] * r[i] * r[i])); // SPHERICALLY SYMMETRIC EXPRESSION, NOT EXACT

	//	dOmega[i] = - 1.5 * Omega[i] / r[i] + 0.5 * Omega[i] * 
	//	  (two_pi * r[i] * model.Sigma(r[i]) + four_pi * sq(r[i]) * nfw.rho(r[i])) / mass_total[i];
      }
  }
...
ostream & operator<< (ostream & out, const Profile & profile)
{
  for(size_t i = 0; i < profile.size(); i ++)
    {
      out << profile.r[i] << " " << profile.Sigma_baryon[i] << " " << profile.Sigma_total[i] << " " << profile.rho_NFW[i] << " " << profile.mass_baryon[i] << " "<< profile.mass_total[i] << endl; //<< " " << profile.Omega[i] << " " << profile.dOmega[i] << endl;
    }
  return out;
}
...
double timeMigration(double cluster_mass, double r, const Profile & profile)
{
  ...
  static const double G_power = pow(G, 1.5);
  ...
  const double evolving_density = ((profile.evolving_mass_profile[i + 1] - profile.evolving_mass_profile[i]) 
				   / (profile.r[i + 1] - profile.r[i])) / (two_pi * R);

  const double mass = profile.nfw.M(r) + profile.mass_nucleus + max<double> (profile.model.M(r), evolving_mass);

  const double Sigma_disk = max<double> (profile.model.Sigma(R), evolving_density);

  //const double mass = profile.model.M(r) + profile.nfw.M(r) + profile.mass_nucleus;
  const double rho_DM = profile.nfw.rho(r);

  //const double Sigma_disk = profile.model.Sigma(r);
  const double V_circ = sqrt(G * mass / r);
  const double dispersion = 0.2 * V_circ;

  return dispersion * dispersion * pow(r, 1.5) * (two_pi * r * Sigma_disk + four_pi * sq(r) * rho_DM)  / (2.0 * G_power * cluster_mass * sqrt(mass) * Sigma_disk) + 0.5 * dispersion * dispersion * sqrt(mass * r) / (G_power * cluster_mass * Sigma_disk);
}
...
double timeDisruption(double cluster_mass, double r, const Profile & profile)
{
 ...
 if(radius_pericenter < profile.r.front() || radius_pericenter >= profile.r.back()) 
  ...
  const double evolving_density = ((profile.evolving_mass_profile[i + 1] - profile.evolving_mass_profile[i]) 
				   / (profile.r[i + 1] - profile.r[i])) / (two_pi * radius_pericenter);
...
//   const double t_dis = t_4 * pow(cluster_mass / (1.0e4 * M_sun), gamma);
  ...
  // NEW
  const double dOmega = - 1.5 * Omega / r + 0.5 * Omega * (two_pi * r * Sigma_disk + four_pi * sq(r) * profile.nfw.rho(radius_pericenter)) / mass;

  //  const double dOmega = - 1.5 * Omega / r + 0.5 * Omega * (two_pi * r * profile.model.Sigma(radius_pericenter) + four_pi * sq(r) * profile.nfw.rho(radius_pericenter)) / (profile.model.M(radius_pericenter) + profile.nfw.M(radius_pericenter) + profile.mass_nucleus);
...
//   vector<double> time_mig_2(profile.r.size(), 0.0), time_mig_3(profile.r.size(), 0.0), time_mig_4(profile.r.size(), 0.0), time_mig_5(profile.r.size(), 0.0), time_mig_6(profile.r.size(), 0.0), time_mig_7(profile.r.size(), 0.0);
//   vector<double> time_dis_2(profile.r.size(), 0.0), time_dis_3(profile.r.size(), 0.0), time_dis_4(profile.r.size(), 0.0), time_dis_5(profile.r.size(), 0.0), time_dis_6(profile.r.size(), 0.0), time_dis_7(profile.r.size(), 0.0);
//   for(size_t i = 0; i < profile.r.size(); i ++) 
//     {
//       time_mig_2[i] = timeMigration(1.0e2 * M_sun, profile.r[i], profile);
//       time_mig_3[i] = timeMigration(1.0e3 * M_sun, profile.r[i], profile);
//       time_mig_4[i] = timeMigration(1.0e4 * M_sun, profile.r[i], profile);
//       time_mig_5[i] = timeMigration(1.0e5 * M_sun, profile.r[i], profile);
//       time_mig_6[i] = timeMigration(1.0e6 * M_sun, profile.r[i], profile);
//       time_mig_7[i] = timeMigration(1.0e7 * M_sun, profile.r[i], profile);

//       time_dis_2[i] = timeDisruption(1.0e2 * M_sun, profile.r[i], profile);
//       time_dis_3[i] = timeDisruption(1.0e3 * M_sun, profile.r[i], profile);
//       time_dis_4[i] = timeDisruption(1.0e4 * M_sun, profile.r[i], profile);
//       time_dis_5[i] = timeDisruption(1.0e5 * M_sun, profile.r[i], profile);
//       time_dis_6[i] = timeDisruption(1.0e6 * M_sun, profile.r[i], profile);
//       time_dis_7[i] = timeDisruption(1.0e7 * M_sun, profile.r[i], profile);

//       cout << profile.r[i] << " " << time_mig_2[i] << " " << time_mig_3[i] << " " << time_mig_4[i] << " " << time_mig_5[i] << " " << time_mig_6[i] << " " << time_mig_7[i] << " " << time_dis_2[i] << " " << time_dis_3[i] << " " << time_dis_4[i] << " " << time_dis_5[i] << " " << time_dis_6[i] << " " << time_dis_7[i] << endl; 
//     }
//   abort();
  ...
  vector<double> mass_center(clusters.size(), 0.0), mass_center_excess(clusters.size(), 0.0), mass_center_excess_actual(clusters.size(), 0.0);//, time_center(clusters.size(), 0.0), time_center_actual(clusters.size(), 0.0);
...
ofstream center_yes_out("data/script/center_yes_disk_1.0e+07_high_z.out");
ofstream center_no_out("data/script/center_no_disk_1.0e+07_high_z.out");
  for(size_t i = 0; i < clusters.size(); i ++)
    {
      //const double half_cluster_mass = 0.5 * clusters[i].mass;
	...
	if (profile.r[j] > r_cluster) 
      ...
      //double deposited_cluster_mass = 0.0;
      for(size_t j = 1; j < dM_dr.size(); j ++)
	{
	  ...
	  mass_deposited += delta_mass;
 	  ...
 	  // deposited_cluster_mass += delta_mass; 
	  
	  // if ((deposited_cluster_mass + dM_dr.disrupt_mass + dM_dr.center_mass) < half_cluster_mass)
	  //   {
	  //     for(size_t k = 0; k < profile.r.size(); k ++) 
	  // 	if (profile.r[k] > r) final_mass[k] += delta_mass;
      ...
      for(size_t j = 0; j < profile.r.size(); j ++)
      	if (profile.r[j] > dM_dr.disrupt_radius)
      	  {
      	    //final_mass_disrupt[j] += min<double> (dM_dr.disrupt_mass, half_cluster_mass);
	    final_mass_disrupt[j] += dM_dr.disrupt_mass;
      	  }
      
      //mass_nucleus += min<double> (dM_dr.center_mass, half_cluster_mass);
      //mass_disrupted += min<double> (dM_dr.disrupt_mass, half_cluster_mass);
  ...
  // ofstream center_out("center.dat");
  ...
  for(size_t i = 0; i < profile.r.size(); i ++)
    {
      final_mass_total[i] = mass_nucleus + final_mass_disrupt[i] + final_mass[i]; //add mass that fell into nucleus, mass that remained after disruption, and smeared mass
    }

  // vector<double> final_rho_nuc(profile.r.size(), 0.0);  
  // const double r_nucleus = 5.0 * pc;  
  ...
  vector<double> final_Sigma_nuc(profile.r.size(), 0.0);
  const double r_nucleus = 2.0 * pc;  
  for(size_t i = 0; i < profile.r.size(); i ++)
    {
      final_Sigma_nuc[i] += mass_nucleus * sq(r_nucleus) / (M_PI * sq(sq(r_nucleus) + sq(profile.r[i])));
    }

  vector<double> initial_Sigma(profile.r.size()), final_Sigma(profile.r.size()), final_Sigma_disrupt(profile.r.size()), final_Sigma_deposit(profile.r.size());
  for(size_t i = 1; i < final_mass.size() - 1; i += 1)
    {
      initial_Sigma[i] =((initial_mass[i + 1] - initial_mass[i - 1]) / (profile.r[i + 1] - profile.r[i - 1])) / (two_pi * (0.5 * (profile.r[i + 1] + profile.r[i - 1])));
      final_Sigma[i] = ((final_mass_total[i + 1] - final_mass_total[i - 1]) / (profile.r[i + 1] - profile.r[i - 1])) / (two_pi * (0.5 * (profile.r[i + 1] + profile.r[i - 1])));
      final_Sigma_disrupt[i] = ((final_mass_disrupt[i + 1] - final_mass_disrupt[i - 1]) / (profile.r[i + 1] - profile.r[i - 1])) / (two_pi * (0.5 * (profile.r[i + 1] + profile.r[i - 1])));
      final_Sigma_deposit[i] = ((final_mass[i + 1] - final_mass[i - 1]) / (profile.r[i + 1] - profile.r[i - 1])) / (two_pi * (0.5 * (profile.r[i + 1] + profile.r[i - 1])));
    }
  initial_Sigma[0] = initial_Sigma[1];
  initial_Sigma[profile.r.size() - 1] = initial_Sigma[profile.r.size() - (2 * 1)];
  final_Sigma[0] = final_Sigma[1];
  final_Sigma[profile.r.size() - 1] = final_Sigma[profile.r.size() - (2 * 1)];
  final_Sigma_disrupt[0] = final_Sigma_disrupt[1];
  final_Sigma_disrupt[profile.r.size() - 1] = final_Sigma_disrupt[profile.r.size() - (2 * 1)];
  final_Sigma_deposit[0] = final_Sigma_deposit[1];
  final_Sigma_deposit[profile.r.size() - 1] = final_Sigma_deposit[profile.r.size() - (2 * 1)];
 ...
 for(size_t i = 0; i < final_mass.size(); i += step)
   {
     cout << profile.r[i] << " " 
	  << profile.Sigma_NFW[i] << " " 
