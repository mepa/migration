...
for ($Ms = 1.0e4; $Ms < 1.0e8; $Ms=$Ms*10) 
{
    ...
    $outfile = 'main_'.$galaxy.'_'.$redshift.'_'.$M_max.'.cc';
    open(OFILE, ">$outfile");  

    $cluster_out = 'cluster_'.$galaxy.'_'.$M_max.'_'.$redshift.'_z.out';
	...
	if ($line =~ /ofstream cluster_out\(/) {
	    $line = "ofstream cluster_out(\"../Data/Run1_Standard/$cluster_out\");\n";
	}
	
	if ($line =~ /ofstream center_out\(/) {
	    $line = "ofstream center_out(\"../Data/Run1_Standard/$center_out\");\n";
	}
	if ($line =~ /ofstream center_yes_out\(/) {
	    $line = "ofstream center_yes_out(\"../Data/Run1_Standard/$center_yes_out\");\n"; 
	} 
	if ($line =~ /ofstream center_no_out\(/) {
	    $line = "ofstream center_no_out(\"../Data/Run1_Standard/$center_no_out\");\n";  
	}
    ...
    $datafile = 'profile_'.$galaxy.'_'.$mass.'_'.$mass_halo.'_'.$M_max.'_'.$redshift.'_z_low_res.dat';
    print `make\n`;
    print `$exe > ../Data/Run1_Standard/$datafile &\n`;
    print `cp -p $outfile ../Data/Run1_Standard/ &\n`;  
}
