...
$code =  'main_'.$galaxy.'_mass_loss.cc';
open(CFILE, "<$code");
@clines = <CFILE>;
close(CFILE);
...
for ($Ms = 1.0e4; $Ms < 1.0e8; $Ms=$Ms*10) 
{
    ...
    $outfile = 'main_'.$galaxy.'_mass_loss_'.$redshift.'_'.$M_max.'.cc';
    open(OFILE, ">$outfile");  
	...
	if ($line =~ /ofstream center_out\(/) {
	    $line = "ofstream center_out(\"../Data/Run2_WithMassLoss/$center_out\");\n";
	}
	if ($line =~ /ofstream center_yes_out\(/) {
	    $line = "ofstream center_yes_out(\"../Data/Run2_WithMassLoss/$center_yes_out\");\n"; 
	} 
	if ($line =~ /ofstream center_no_out\(/) {
	    $line = "ofstream center_no_out(\"../Data/Run2_WithMassLoss/$center_no_out\");\n";  
	}
    ...
    $datafile = 'profile_'.$galaxy.'_mass_loss_'.$mass.'_'.$mass_halo.'_'.$M_max.'_'.$redshift.'_z_low_res.dat';
    print `make\n`;
    print `$exe > ../Data/Run2_WithMassLoss/$datafile &\n`;
    print `cp -p $outfile ../Data/Run2_WithMassLoss/ &\n`;  
}
